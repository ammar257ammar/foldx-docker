...
for f in *; do
	...
	if [[ "$counter" -lt $THREADS ]]; then
		...
		if [ "$COMMAND" == "RepairPDB" ]; then
			...
			/foldx/foldx --command=BuildModel --pdb=${f}_protein_Repair.pdb --mutant-file=individual_list.txt --ionStrength=0.05 --pH=7 --water=CRYSTAL --vdwDesign=2 --output-dir=../output --out-pdb=true --pdbHydrogens=false --numberOfRuns=3 > ../output/log.txt &
